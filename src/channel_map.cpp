...
ValidationResult validate_channel_map(const ChannelMap& map,const FrameSource& target){
  ...
  if(!alpha_opaque)add("SPRITE_CHANNEL_ALPHA_INVALID","channel maps require fully opaque storage alpha");
  if((map.kind==ChannelMapKind::depth||map.kind==ChannelMapKind::effects||map.kind==ChannelMapKind::collision)&&!grayscale)add("SPRITE_CHANNEL_GRAYSCALE_REQUIRED","scalar channel map must use equal RGB channels");
  if(map.kind==ChannelMapKind::collision&&!binary)add("SPRITE_CHANNEL_BINARY_REQUIRED","collision mask must contain only 0 or 255");
  if(map.kind==ChannelMapKind::material_id&&!material_encoding)add("SPRITE_CHANNEL_MATERIAL_ENCODING_INVALID","material IDs must be stored in R with G and B zero");
  if(map.kind==ChannelMapKind::tangent_normal&&!normals_unit)add("SPRITE_CHANNEL_NORMAL_INVALID","tangent normals must face outward and have near-unit length");
  return result;
}

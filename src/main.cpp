...
int main(int argc, char** argv) {
  try {
    if(argc==3&&std::string_view(argv[1])=="verify"){
      ...
      std::cout<<"verified "<<verification.entity_id<<" identity="<<verification.seed_identity<<" artifacts="<<verification.artifact_count<<" bytes="<<verification.total_artifact_bytes<<'\n';return 0;
    }

...
std::vector<TemporalTransitionMetrics> analyze_temporal_stability(std::span<const FrameSource> frames,std::uint8_t threshold,std::uint64_t maximum_pixels){
  if(frames.size()<2||maximum_pixels==0||maximum_pixels>std::numeric_limits<std::uint64_t>::max()/1'000'000ULL)throw std::invalid_argument("temporal analysis requires at least two frames and a bounded positive comparison limit");for(const auto&frame:frames){require_image(frame.image);if(frame.id.empty())throw std::invalid_argument("temporal frame identity is empty");}

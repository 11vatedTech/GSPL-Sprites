...
The tests/evidence_oracle.json file SHALL contain the expected deterministic output for the headless evidence scenario (seed=42, SpriteIr from headless.test seed). The headless_evidence test SHALL verify that current output matches the oracle byte-for-byte.

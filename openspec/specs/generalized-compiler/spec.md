...
The SpriteSeed→SpriteIr→synthesis pipeline accepts any valid GSPL seed (not only Voltfox) and produces typed, validated intermediate-representation records with deterministic output.
...
- **THEN** the result SHALL contain non-empty 2D, 2.5D, and 3D representations with frame hashes on every 2D FrameSource

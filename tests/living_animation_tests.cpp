...
void test_k_required_clips() {
  ...
  TEST("kRequiredClips has base_idle", ids.contains("base_idle"));
  TEST("kRequiredClips has base_locomotion", ids.contains("base_locomotion"));
  TEST("kRequiredClips has base_attack", ids.contains("base_attack"));
  TEST("kRequiredClips has base_hit", ids.contains("base_hit"));
  TEST("kRequiredClips has transform_ascend", ids.contains("transform_ascend"));
  TEST("kRequiredClips has storm_idle", ids.contains("storm_idle"));
  TEST("kRequiredClips has storm_locomotion", ids.contains("storm_locomotion"));
  TEST("kRequiredClips has storm_attack", ids.contains("storm_attack"));
  TEST("kRequiredClips has storm_hit", ids.contains("storm_hit"));
...
void test_resolve_form_morphology() {
  ...
  TEST("base torso color", base.at("torso").color == "#4488CC");
  ...
  // Base and storm must differ structurally
...
void test_canonical_identity() {
  ...
  SpriteSeed seed2 = seed;
  auto c1 = canonicalize(seed);
  auto c2 = canonicalize(seed2);
  TEST("deterministic identity", c1 == c2);

  // Morphology -> changes identity
  ...
  // Change bone_id -> identity changes
  ...
  // Change primitive -> identity changes
  ...
  // Change z_order -> identity changes
  ...
  // Change semantic_role -> identity changes
  ...
  // Form override -> identity changes
  MorphologyPart storm_torso;
  ...
  // Storm ability -> identity changes
  auto seed_sa = seed;

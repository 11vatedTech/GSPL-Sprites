...
Projection3dDefinition projection(std::string id) { Projection3dDefinition p; p.id=std::move(id);
  p.materials={{"material",0xffffffffU,0,1'000'000,MaterialAlphaMode::opaque,500'000,false,{},{},{}}};
...
int main() try {
  ...
  TransformationState state{"entity","base",10,10,std::nullopt,{}}; begin_transformation(tp,cp,state,"up",10);
